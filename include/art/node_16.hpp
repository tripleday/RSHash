...
template <class T> inner_node<T> *node_16<T>::grow() {
  ...
  for (int i = 0; i < n_children_; ++i) {
    // 确保键值在有效范围内
    unsigned char index = static_cast<unsigned char>(128 + this->keys_[i]);
    if (index >= 256) {
      // 处理错误情况
      delete new_node;
      throw std::runtime_error("Invalid key value in node16 grow");
    }
    new_node->indexes_[index] = i;
  }
  
  // 验证转换的正确性
  for (int i = 0; i < n_children_; ++i) {
    char key = this->keys_[i];
    if (new_node->find_child(key) == nullptr) {
      // 转换失败，回滚
      delete new_node;
      throw std::runtime_error("Node16 to Node48 conversion failed");
    }
  }
