...
template <typename RSs, typename key_t>
void RSBenchmark(std::vector<key_t> &inputData, std::string &dataset, size_t numQueries) {
    std::cout<<"\r\033[K"<<"Generating "<<RSs::name()<<std::flush;
    ...
    std::cout<<"\r\033[K"<<"Verifying "<<RSs::name()<<std::flush;
    for (size_t i = 0; i < inputData.size(); i++) {
        if (RS(inputData.at(i)) != i) {
            std::cerr<<std::endl<<std::endl<<"Error verifying key "<<i<<std::endl;
        }
    }

    std::cout<<"\r\033[K"<<"Benchmarking "<<RSs::name()<<std::flush;
