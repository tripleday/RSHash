...
template <typename ARTs, typename key_t>
void artBenchmark(std::vector<key_t> &inputData, std::string &dataset, size_t numQueries) {
    std::cout<<"\r\033[K"<<"Generating "<<ARTs::name()<<std::flush;
    ...
    std::cout<<"\r\033[K"<<"Verifying "<<ARTs::name()<<std::flush;
    for (size_t i = 0; i < inputData.size(); i++) {
        if (art(inputData.at(i)) != i) {
            std::cout<<art(inputData.at(i))<<std::endl;
            std::cerr<<std::endl<<std::endl<<"Error verifying key "<<i<<std::endl;
        }
    }

    std::cout<<"\r\033[K"<<"Benchmarking "<<ARTs::name()<<std::flush;
