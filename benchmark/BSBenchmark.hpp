...
template <typename BSs, typename key_t>
void BSBenchmark(std::vector<key_t> &inputData, std::string &dataset, size_t numQueries) {
    std::cout<<"\r\033[K"<<"Generating "<<BSs::name()<<std::flush;
    ...
    std::cout<<"\r\033[K"<<"Verifying "<<BSs::name()<<std::flush;
    for (size_t i = 0; i < inputData.size(); i++) {
        if (BS(inputData.at(i)) != i) {
            std::cerr<<std::endl<<std::endl<<"Error verifying key "<<i<<std::endl;
        }
    }

    std::cout<<"\r\033[K"<<"Benchmarking "<<BSs::name()<<std::flush;
