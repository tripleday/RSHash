...
template <typename RSHashs, typename key_t>
void RSHashBenchmark(std::vector<key_t> &inputData, std::string &dataset, size_t numQueries) {
    std::cout<<"\r\033[K"<<"Generating "<<RSHashs::name()<<std::flush;
    ...
    std::cout<<"\r\033[K"<<"Verifying "<<RSHashs::name()<<std::flush;
    for (size_t i = 0; i < inputData.size(); i++) {
        if (RSHash(inputData.at(i)) != i) {
            std::cerr<<std::endl<<std::endl<<"Error verifying key "<<i<<std::endl;
        }
    }
    // elements not in the set
    // for (size_t i = 1; i < 11; i++) {
    //     std::cout<<RSHash(i)<<std::endl;
    // }

    std::cout<<"\r\033[K"<<"Benchmarking "<<RSHashs::name()<<std::flush;
